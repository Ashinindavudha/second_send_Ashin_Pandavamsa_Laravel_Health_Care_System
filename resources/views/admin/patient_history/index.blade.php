      ...
      <table class="table table-striped table-hover datatable datatable-Role">
        ...
        <tbody>
          ...
          <tr data-entry-id="{{ $treatment->id }}">
            ...
            <td>
              @foreach($treatment->diseases as $key => $item)
            ...
            <td>
              ...
              @can('treatment_edit')
              <a class="btn btn-xs btn-info" href="{{ route('histories.edit', $treatment->id) }}">
                {{ trans('global.edit') }}
              </a>
              @endcan

              @can('treatment_delete')
              <form action="{{ route('histories.destroy', $treatment->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
