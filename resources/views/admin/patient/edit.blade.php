    ...
    <div class="card-body">
        <form action="{{ route('patients.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group {{ $errors->has('age') ? 'has-error' : '' }}">
                ...
                <input type="text" id="age" name="age" class="form-control" value="{{ old('age', $patient->age) }}" required>
                @if($errors->has('age'))
                <p class="help-block">
                    {{ $errors->first('age') }}
                </p>
            ...
            <div class="form-group {{ $errors->has('diseases') ? 'has-error' : '' }}">
                <label for="diseases">{{ trans('cruds.western_doctor.fields.disease') }}*
                    ...
                    <select name="diseases[]" id="diseases" class="form-control select2" multiple="multiple" required>
                        ...
                        <option value="{{ $id }}" {{ (in_array($id, old('diseases', [])) || $patient->diseases->contains($id)) ? 'selected' : '' }}>{{ $disease }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group {{ $errors->has('phone') ? 'has-error' : '' }}">
                    ...
                    <input type="text" id="phone" name="phone" class="form-control" value="{{ old('phone', $patient->phone) }}">
                    @if($errors->has('phone'))
                    <p class="help-block">
                        {{ $errors->first('phone') }}
                    </p>
                ...
                <div class="form-group {{ $errors->has('remark') ? 'has-error' : '' }}">
                    ...
                    <textarea id="remark" name="remark" class="form-control">{!! old('remark', $patient->remark) !!}</textarea>
