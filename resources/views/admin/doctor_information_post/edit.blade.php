            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('diseases') ? 'is-invalid' : '' }}" name="diseases[]" id="diseases" multiple>
                    @foreach($diseases as $id => $disease)
                        <option value="{{ $id }}" {{ (in_array($id, old('diseases', [])) || $post->diseases->contains($id)) ? 'selected' : '' }}>{{ $disease }}</option>
                    @endforeach
                </select>
...
<script>
    ...
    init: function () {
@if(isset($post) && $post->logo)
      var file = {!! json_encode($post->logo) !!}
          this.options.addedfile.call(this, file)
      this.options.thumbnail.call(this, file, '{{ $post->logo->getUrl('thumb') }}')
