...
@can('treatment_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('treatments.create') }}">
                ...
                <tbody>
                    @foreach($treatments as $key => $treatment)
                        <tr data-entry-id="{{ $treatment->id }}">
                            ...
                            <td>
                                @can('role_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('treatments.show', $treatment->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('role_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('treatments.edit', $treatment->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('role_delete')
                                    <form action="{{ route('treatments.destroy', $treatment->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
